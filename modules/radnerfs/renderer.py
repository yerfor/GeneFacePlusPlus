...
class NeRFRenderer(nn.Module):
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        if hparams.get("to_heatmap", False):
            from tasks.radnerfs.dataset_utils import transform_normed_lm_to_pixel_lm, convert_to_tensor, get_ldm_heatmap
            inds = torch.arange(512*512)
            lm478 = cond.reshape([hparams['smo_win_size'], 478, 3])
            lm478_px = convert_to_tensor(transform_normed_lm_to_pixel_lm(lm478)).reshape([hparams['smo_win_size'], 1, 478, 3])
            heatmaps = []
            for i in range(hparams['smo_win_size']):
                lm = lm478_px[i:i+1]
                # heatmap = get_ldm_heatmap([512,512], lm.cuda()).cpu() # [1, 1, W, H, C=478]
                heatmap = get_ldm_heatmap([512,512], lm.cuda()) # [1, 1, W, H, C=478]
                heatmaps.append(heatmap.reshape([1, 512, 512, 478]))
            heatmaps = torch.stack(heatmaps) # [5,1,W,H,C]
            heatmaps = heatmaps.reshape([hparams['smo_win_size'], 1, 512*512, 478])
            cond = heatmaps # [:, :, :, :]
            enc_a = self.cal_cond_feat(cond.to(self.density_bitfield.device))
            del cond
        ...
        for xs in X:
            for ys in Y:
                for zs in Z:
                    ...
                    # cascading
                    if hparams.get("to_heatmap", False) is True:
                        enc_a_ = enc_a[torch.randint(low=0, high=len(enc_a), size=(len(xyzs),))]
    ...
    def render(self, rays_o, rays_d, cond, bg_coords, poses, index=0, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, cond_mask=None,eye_area_percent=None,**kwargs):
        ...
        if self.training:
            ...
            align = 128
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, self.bound, self.density_bitfield, self.cascade, self.grid_size, nears, fars, counter, self.mean_count, perturb, align, force_all_rays, dt_gamma, max_steps)
            # xyzs, dirs, deltas, rays, points2rays = raymarching.march_rays_train(rays_o, rays_d, self.bound, self.density_bitfield, self.cascade, self.grid_size, nears, fars, counter, self.mean_count, perturb, align, force_all_rays, dt_gamma, max_steps)
            
            sigmas, rgbs, ambient = self.forward(xyzs, dirs, cond_feat, ind_code, cond_mask=cond_mask)
            ...
            while step < max_steps:
                ...
                rays_alive_prev = copy.deepcopy(rays_alive)
                ...
                if cond_mask is not None:
                    ...
                    sigmas, rgbs, ambient = self.forward(xyzs, dirs, cond_feat, ind_code)
