...
class GridEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=None, gridtype='hash', align_corners=False, interpolation='linear'):
        super().__init__()
        """
        resolution: number of grids on a axis. If this is a 2D grid, then the grid shape is [resolution,resolution]
        log2_hashmap_size: log2 of the maximum number of grids in a level, for 2D grid, we have 2**log2_hashmap >= resolution ** 2
        """
        ...
        # parameters
        self.embeddings = nn.Parameter(torch.empty(offset, level_dim)) # save the embedding of each grid in each level.
...
if __name__ == '__main__':
    grid_encoder = GridEncoder(gridtype='tiled', desired_resolution=2048, log2_hashmap_size=16)
    print(" ")
